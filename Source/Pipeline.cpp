...
namespace lv
{
    ...
    Pipeline::Pipeline(Device& device, const PipelineConfigInfo& pipelineConfigInfo,
           const std::filesystem::path& vertexPath, const std::filesystem::path& fragmentPath)
       : m_Device(device)
    {
        CreateGraphicsPipeline(vertexPath, fragmentPath, pipelineConfigInfo);
    }
    ...
    void Pipeline::CreateGraphicsPipeline(const std::filesystem::path& vertexPath, const std::filesystem::path& fragmentPath,
                                          const PipelineConfigInfo& pipelineConfigInfo)
    {
        ASSERT(pipelineConfigInfo.pipelineLayout != VK_NULL_HANDLE, "Cannot create graphics pipeline, no pipelineLayout provided in pipelineConfigInfo")
        ASSERT(pipelineConfigInfo.renderPass != VK_NULL_HANDLE, "Cannot create graphics pipeline, no renderPass provided in pipelineConfigInfo")

        auto vert = ReadFile(vertexPath);
        auto frag = ReadFile(fragmentPath);

        CreateShaderModule(vert, &m_VertShaderModule);
        CreateShaderModule(frag, &m_FragShaderModule);
        ...
        auto bindingDescriptions = lv::Model::Vertex::GetBindingDescriptions();
        auto attributeDescriptions = lv::Model::Vertex::GetAttributeDescriptions();
    ...
    void Pipeline::CreateShaderModule(const std::vector<char>& source, VkShaderModule* shaderModule)
    {
